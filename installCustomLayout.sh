#!/bin/bash
# Replace the "English (Macintosh)" Layout with a custom one.

TIMEST=$(date +%Y%m%d-%k%M)
DESTI=/usr/share/X11/xkb/symbols

sudo mkdir -p /var/lib/xkb/bak.$TIMEST && \
sudo mv /var/lib/xkb/*.xkm /var/lib/xkb/bak.$TIMEST/ || true && \
sudo cp "$DESTI/us" "$DESTI/us.$TIMEST" && \
sudo tee "$DESTI/us" <<'EOF'
...
xkb_symbols "mac" {
    ...
    //              norm    shift           alt_gr      shift+altgr
    key <LSGT> { [  less,    greater, greaterthanequal, lessthanequal ] };
    key <TLDE> { [ grave, asciitilde,    dead_grave,       exclamdown ] };
    key <AE01> { [     1,     exclam,   onesuperior,            U2081 ] }; // sub- and superscript one
    key <AE02> { [     2,         at,   twosuperior,            U2082 ] };
    key <AE03> { [     3, numbersign, threesuperior,            U2083 ] };
    key <AE04> { [     4,     dollar,          cent,         EuroSign ] };
    key <AE05> { [     5,    percent,      infinity,         sterling ] };
    key <AE06> { [     6,asciicircum,       section,            U00D7 ] }; // multiplication sign
    key <AE07> { [     7,  ampersand,     paragraph,     doubledagger ] };
    key <AE08> { [     8,   asterisk, enfilledcircbullet,      degree ] };
    key <AE09> { [     9,  parenleft,periodcentered,            U2102 ] }; // complex numbers
    key <AE10> { [     0, parenright,  zerosuperior,            U2080 ] }; // superscrit zero
    key <AE11> { [     minus, underscore,    endash,           emdash ] };
    key <AE12> { [     plus,       equal,  notequal,        plusminus ] };

    key <AD01> { [     q,          Q,   Greek_alpha,      Greek_THETA ] };
    key <AD02> { [     w,          W,   Greek_omega,      Greek_OMEGA ] };
    key <AD03> { [     e,          E, Greek_epsilon,      Greek_SIGMA ] };
    key <AD04> { [     r,          R,     Greek_rho,       registered ] };
    key <AD05> { [     t,          T,     Greek_tau,           dagger ] };
    key <AD06> { [     y,          Y,           yen,          notsign ] };
    key <AD07> { [     u,          U,    udiaeresis,       Udiaeresis ] };
    key <AD08> { [     i,        I, dead_circumflex,            U2203 ] }; // natural numbers
    ...
    key <AC04> { [     f,          F,     Greek_psi,        Greek_PSI ] };
    key <AC05> { [     g,          G,   Greek_gamma,      Greek_GAMMA ] };
    key <AC06> { [     h,          H,         U2208,            U2209 ] }; // element of + not element of
    key <AC07> { [     j,          J,         U2206,      Greek_sigma ] };
    key <AC08> { [     k,          K,   Greek_kappa,            U221D ] }; // rational numbers

    key <AC09> { [     l,          L,  Greek_lambda,     Greek_LAMBDA ] };
    key <AC10> { [ semicolon,      colon,     U2026,             211D ] }; // ... and real numbers
    key <AC11> { [apostrophe,   quotedbl,     U1D60,        Greek_PHI ] };

    key <AB01> { [     z,          Z,     copyright,        trademark ] };
    key <AB02> { [     x,          X,      Greek_xi,         Greek_XI ] };
    key <AB03> { [     c,          C,    Greek_zeta,            U03D1 ] };
    key <AB04> { [     v,          V,    squareroot,         Greek_nu ] };
    key <AB05> { [     b,          B,    Greek_beta,       dead_tilde ] };
    key <AB06> { [     n,          N,     Greek_eta,            U2115 ] }; // it exists
    key <AB07> { [     m,          M,      Greek_mu,            U2200 ] }; // for all
                    // unclear whether "approxeq" is 2248 or 2245
    key <AB08> { [ comma,       less,         U2248,             U2207 ] }; // ??? and Nabla
    key <AB09> { [ period,     U00B7,      integral, partialderivative ] };
...
//-----------------------------------------------------------------------------
// ... AND HERE THE RELEVANT PART ENDS
///////////////////////////////////////////////////////////////////////////////
